...
Since we were not able to retrieve an existing dataset of traces of realtime editing sessions, we generated and used random traces to evaluate our data structure.
To produce the random traces, we set up collaborative editing sessions of 10 bots.
Every 200ms, each bot performs either an \emph{insert} or a \emph{remove} operation and broadcast it to other bots.
The collaboration ends once each bot observes every operation.

In our settings, each bot performs 50k operations with a ratio of 80/20\% of \emph{insert}/\emph{remove} operations.
Bots also have 5\% of chance to move their cursor to a new position before performing an operation.
We take snapshots of the data structure each 50k steps to follow its evolution.

The code used to generate the traces can be found at the following address: \url{https://github.com/coast-team/mute-bot-random/}.
This repository also contains the code corresponding to the experiments described in the next subsections as well as the results obtained.

\subsection{Overhead of the data structure}

% \begin{itemize}
%     \item In our experimental settings, the overhead introduced by LogootSplit is close to linear to the number of elements of the sequence.
%     \item Results in a document size of 105Mo after applying 500k operations.
%     \item Can be explain by our experimental settings: as bots remove 1 character for each 4 characters inserted, it prevents them to generate long blocks which would benefit from the
%     \item
% \end{itemize}
