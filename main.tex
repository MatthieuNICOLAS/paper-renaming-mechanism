...
\begin{itemize}
    ...
    \end{itemize}
    \item Add the new operation $rename$
    \item Have to be able to deal with concurrent operations
    \begin{itemize}
        \item As only the leader can perform renaming, cannot exist a concurrent $rename$ operation...
    ...
    \end{itemize}
    \item Have to broadcast the $rename$ operation so other nodes can also update their own state
...
\begin{itemize}
    \item Consist in generating an equivalent Sequence, with the same content but composed of only one block
    \item Need to generate a $position \ interval$ for this block
    \item Have to pick a position as the beginning of the interval
    \item This choice is arbitrary
    \item In our case, generate the following position $pos$:

    \begin{center}
        $pos = <priority, id_{site}, seq_{site}, 0>$
    \end{center}

    where:

    \begin{itemize}
        \item $priority$ is the same value as the $priority$ of the first tuple of the first element of the sequence. We explain this choice in the next section.
        \item $id_{site}$ is the $id_{site}$ of the node performing the $rename$ operation
        \item $seq_{site}$ is the current $seq_{site}$ of the node performing the $rename$ operation
    \end{itemize}

    \item Using the position $pos$, we are able to generate the following position interval $posInterval$:
    ...
    \item Using $posInterval$ and the content of the sequence, we are able to generate an equivalent sequence $S'$ but optimised to replace the current state
\end{itemize}

\mnnote{TODO: ajouter l'algorithme correspondant en pseudo-code}
...
\begin{itemize}
    \item As stated previously, the renaming generates additional metadata: the epochs and the rewriting rules used to transform concurrent operations against the renaming
    ...
    \begin{itemize}
        ...
        \item We can then safely garbage collect all previous epochs and corresponding rewriting rules
    \end{itemize}
\end{itemize}
