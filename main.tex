...
% Ajout de l'EpochStore
% - Structure arborescente
%   - Permet de maintenir la liste des epochs
%   - Et pour chaque epoch, la liste des renamedBlocks
% - Permet de récupérer les informations à utiliser pour transformer une opération
%    - Une ou plusieurs transformations à appliquer
%    - Nécessité de chemin entre epochs

% We have to keep track of epochs and their corresponding \emph{renamedBlocks}.
% Indeed, nodes may receive concurrent operations to \emph{rename} operations.
% They need to store this data to transform these operations.
% Moreover, some of these operations can be concurrent to several \emph{rename} operations.
% We thus have to be able to compute the list of epochs separating two others.

% To this end, we add a new data structure to the replicated sequence, the \emph{Epoch Store}.
% This arborescent data structure stores a node for each \emph{rename} operation.
% Nodes are identified by the epoch they correspond to and store the corresponding \emph{renamedBlocks}.

% \subsection{Applying \emph{rename} operations}

% Upon the delivery of a \emph{rename} operation, nodes follow these steps to process it.

% First, nodes compute the new epoch based on the $id_{site}$ and $seq_{site}$ embedded in the \emph{rename} operation.
% Then, nodes store

% To this end, we add a new data structure to the replicated sequence, the the \emph{Epoch Store}.
% Nodes register into the \emph{Epoch Store} each epoch and the corresponding \emph{renamedBlocks}.
% The newly inserted epoch is linked to the previous epoch by a parent-child relationship.
...
Its behavior, which is detailed in algorithm \ref{alg:renameForwardPos}, can be described as follow.
...
The first strategy is applied to positions which are included in the interval $]firstPos, maxLastPos[$ where $maxLastPos$ corresponds to the greatest position between $lastPos$ and $newLastPos$.
...
The second case is for positions such as we have $newFirstPos < pos < firstPos$.
...
\begin{algorithm}
    ...
    \Function{renameForwardPos}{$pos: P, newId: \mathbb{I}, newSeq: \mathbb{N}, renamedBlocks: \mathlist{b}{B}$}{: $P$}
        ...
        %     \State \Return $pos$
        %     \Comment Return the position unchanged as it does not interfere with the renaming
        % \EndIf
        % \State
        ...
        % \State $indexOfPredecessorOfPos \gets findIndexOfPredecessor(pos, renamedBlocks)$
        % \State $newPredecessorOfPos \gets new \ P(newPriority, newId, newSeq, indexOfPredecessorOfPos)$
        % \State \Return $concat(newPredecessorOfPos, pos)$

        %%%%%%% VERSION 2 %%%%%%%%%
        \If{$firstPos < pos$ \textbf{and} $pos < maxLastPos$}
        ...
        \If{$newFirstPos < pos$ \textbf{and} $pos < firstPos$}
            \State $predecessorOfNewFirstPos \gets new \ P(newPriority, newId, newSeq, -1)$
            \State \Return $concat(predecessorOfNewFirstPos, pos)$
        \EndIf\\
...
\begin{figure}
    \centering
    \includegraphics[width=0.85\textwidth]{img/filler-example-renameforwardpos.JPG}
...
\subsection{Applying concurrent \emph{insert} and \emph{remove} operations}
