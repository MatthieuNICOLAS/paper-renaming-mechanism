...
\begin{itemize}
    ...
    \item This could prove costly, as the state before renaming can be composed of blocks, each using long identifiers
    ...
    \item Despite the variable length of identifiers, the parts required to identify an identifier uniquely are fixed
    \begin{itemize}
        \item We only need the $siteId$ and the $seq$ of the last tuple of the identifier to do so
    \end{itemize}
    \item Instead of broadcasting the list of whole identifiers, the node which performs the renaming can just broadcast the list of tuples $<siteId, seq>$
    ...
    \item To achieve so, it can browse its current state looking for identifiers with corresponding tuples $<siteId, seq>$
    \item If some identifiers are missing from the state, it means that they were deleted concurrently
    \item The node can thus browse the concurrent remove operations to the renaming one to find the missing blocks
    \item Once all identifiers has been retrieved and the list of blocks computed, the renaming operation can be processed normally
\end{itemize}
