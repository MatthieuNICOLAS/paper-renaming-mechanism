...
\begin{figure}[h]
    ...
    &\fnspec{get}{\set{s \in S} \times \set{ n \in \mathbb{N} \ssep n < length(s) } \to V}{Returns the value from the sequence $s$ at the index $n$}\\
    &\fnspec{insert}{\set{s \in S} \times \set{ n \in \mathbb{N} \ssep n < length(s) } \times V \to S}{Inserts the given value into the sequence $s$ at the index $n$ and returns the updated sequence}\\
    &\fnspec{remove}{\set{s \in S} \times \set{ n \in \mathbb{N} \ssep n < length(s) } \to S}{Removes the value from the sequence $s$ at the index $n$ and returns the updated sequence}\\
    \end{align*}
...
In this example, two nodes A and B own initially a copy the same sequence.
...
These works eventually lead to new specifications of the \emph{Sequence} belonging to a new family of data types: \acfp{CRDT}. \mnnote{TODO: Ajouter référence à WOOT}
...
\acf{SEC} is a consistency model which guarantees that any two nodes of the distributed system observing the same set of updates reach equivalent states, without requiring any other communications than the ones needed to broadcast the updates.
\end{definition}
...
The second one avoids the need of tombstones by instead attaching dense identifiers to values.
It is then able to order values into the sequence by comparing their respective identifiers.
However this approach also suffers from an increasing overhead, as the size of such dense identifiers is variable and grows over time.
