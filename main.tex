...
\begin{figure}[h]
    ...
    &\toto{constructor}{\left( \right) \to S}{Generates and returns an empty sequence}\\
    &\toto{length}{S \to \mathbb{N}}{Returns the number of values contained in the sequence}\\
    &\toto{insert}{\set{s \in S} \times \set{ n \in \mathbb{N} \ssep n < length(s) } \times V \to S}{Inserts the given value into the sequence $s$ at the index $n$ and returns the updated sequence}\\
    &\toto{remove}{\set{s \in S} \times \set{ n \in \mathbb{N} \ssep n < length(s) } \to S}{Removes the value from the sequence $s$ at the index $n$ and returns the updated sequence}\\
    \end{align*}
    \caption{Traditional specification of a sequence}
    \label{fig:spec-seq}
\end{figure}
\mnnote{TODO: Fixer la mise en page pour que la description des opérations ne soient pas tronquées}\\
...
\begin{itemize}
    ...
    \item \mnnote{TODO: Ajouter quelques phrases sur les différentes approches pour les Sequence CRDTs et leurs spécificités. Indiquer qu'on se focalise sur la famille Dense Identifier-based.}
\end{itemize}
